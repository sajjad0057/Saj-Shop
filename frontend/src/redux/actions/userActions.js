...
export const userStatusUpdateAction = (user) => async (dispatch, getState) => {
  try {
    dispatch({
    ...
    console.log("userStatusUpdate --actions ---> user :",user);

    const { data } = await axios.put(`/api/users/update/${user.id}/`,
    user,
    config);
